...
function getSentimentFromBackground(text) {
  return new Promise((resolve, reject) => {
    chrome.runtime.sendMessage(
      { action: "analyzeTweet", tweetText: text },
      (response) => {
...
async function colorizeTweet(tweet) {
  ...
  try {
    ...
    // prev: result comes back as all-caps string of outcome
    // update to: result comes back as
    console.log("back in colorizeTweet", result, result.sentiment);
...
const colorizeTweets = async () => {
  ...
  const tweets = document.querySelectorAll(
    'article[role="article"]:not([data-colored]):not([data-holdAnalysis])'
  );
